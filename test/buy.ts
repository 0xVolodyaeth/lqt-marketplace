...
import { Asset, ItemType, MatchedOrder, OrderType } from "./models";
...
describe("Buy", function () {
	...
	it("Should mint several nfts and exchange them", async function () {
		...
		for (let i = 0; i < leavesLen; i++) {
			const ask: Asset = new Asset(i, ethers.BigNumber.from(1), ItemType.ERC721, testCollectionAsk.address);
			const bid: Asset = new Asset(i, ethers.BigNumber.from(1), ItemType.ERC721, testCollectionBid.address);
		...
		let tokenId = 0
		for (let i = 1; i < buyEvents.length; i += 2) {
			expect(buyEvents[i - 1].args[0]).equal(signer.address)
			expect(buyEvents[i - 1].args[1]).equal(interactor.address)
			expect(buyEvents[i - 1].args[2]).equal(testCollectionBid.address)
			expect(buyEvents[i - 1].args[3]).equal(tokenId.toString())
			expect(buyEvents[i - 1].args[4]).equal("1")

			expect(buyEvents[i].args[0]).equal(interactor.address)
			expect(buyEvents[i].args[1]).equal(signer.address)
			expect(buyEvents[i].args[2]).equal(testCollectionAsk.address)
			expect(buyEvents[i].args[3]).equal(tokenId.toString())
			expect(buyEvents[i].args[4]).equal("1")
			tokenId++;
		}
	...
	it("Should mint several nfts, exchange them, and be reverted because total amount is exceeded", async function () {
		...
		const ask: Asset = new Asset(0, ethers.BigNumber.from(2), ItemType.ERC1155, testCollectionAskERC1155.address);
		const bid: Asset = new Asset(0, ethers.BigNumber.from(2), ItemType.ERC1155, testCollectionBidERC1155.address);
		...
		await expect(marketplace.connect(interactor).buy(matchedOrderLeaves, txExpirationDate, backendSignature, hashes))
			.to.be.revertedWith("Order: position is bought")
	});
	...
	it("Should mint several nfts and exchange them for eth", async function () {
		...
		const expectedSignerBalance = "12.0"
		...
		for (let i = 0; i < leavesLen; i++) {
			...
			const ask: Asset = new Asset(0, ethers.utils.parseEther(priceForNFT.toString()), ItemType.Native, ethers.constants.AddressZero);
			const bid: Asset = new Asset(i, ethers.BigNumber.from(1), ItemType.ERC721, testCollectionBid.address);
		...
		let tokenId = 0
		buyEvents.forEach(x => {
			expect(x.args[0]).equal(signer.address)
			expect(x.args[1]).equal(interactor.address)
			expect(x.args[2]).equal(testCollectionBid.address)
			expect(x.args[3]).equal(tokenId.toString())
			expect(x.args[4]).equal("1")
			tokenId++;
		})
	...
	it("Should mint several nfts and exchange them for currency", async function () {
		...
		let signerExpectedBalance = (4000 + ERC20Amount).toString();

		for (let i = 0; i < leavesLen; i++) {
			tx = await marketplace.connect(backendSigner).changeCollectionRoyalties(testCollectionBid.address, creator.address, ethers.BigNumber.from(250));
			await tx.wait();

			const ask: Asset = new Asset(0, ethers.BigNumber.from(priceForNFTInCurrency), ItemType.ERC20, testToken.address);
			const bid: Asset = new Asset(i, ethers.BigNumber.from(1), ItemType.ERC721, testCollectionBid.address);
	...
	it("Should mint several nfts and exchange them for currency and one for native", async function () {
		...
		let signerExpectedBalance = (3000 + ERC20Amount).toString();

		const expectedSignerBalance = "3.0"
		...
		const askNative: Asset = new Asset(0, ethers.utils.parseEther(priceForNFTInNative.toString()), ItemType.Native, ethers.constants.AddressZero);
	...
	it("Should mint several nfts and exchange them for several currencies", async function () {
		...
		let signerExpectedBalance = (2000 + ERC20Amount).toString();
		...
		matchedOrderLeaves[0].ask = [new Asset(0, ethers.BigNumber.from(priceForNFTInCurrency), ItemType.ERC20, testTokenSecond.address)];
		matchedOrderLeaves[1].ask = [new Asset(0, ethers.BigNumber.from(priceForNFTInCurrency), ItemType.ERC20, testTokenSecond.address)];
	...
	it("Should do SWAP with ERC1155, ERC721 and ERC20 tokens", async function () {
		...
		const bid: Asset[] = [
			new Asset(0, ethers.BigNumber.from(bidAmountOfERC721), ItemType.ERC721, testCollectionBid.address),
			new Asset(0, ethers.BigNumber.from(bidAmountOfERC1155), ItemType.ERC1155, testCollectionBidERC1155.address),
		];

		const ask: Asset[] = [
			new Asset(0, ethers.BigNumber.from(askAmountOfERC20), ItemType.ERC20, testToken.address),
		];
	...
	it("Should make OFFER_TO_MULTI_CURRENCY with ERC721 and two ERC20 tokens", async function () {
		...
		let interactorBalanceExpecrted = (ERC20Amount + priceForERC721Token).toString();
		let marketplaceBeneficiaryBalanceExpected = "4";
		let creatorExpectedBalance = "2";
		let signerExpectedBalance = "9894";
		...
		const ask: Asset[] = [
			new Asset(tokenIdBid, ethers.BigNumber.from(1), ItemType.ERC721, testCollectionAsk.address),
		];

		const bid: Asset[] = [
			new Asset(0, ethers.BigNumber.from(bidAmountOfERC20), ItemType.ERC20, testToken.address),
			new Asset(0, ethers.BigNumber.from(bidAmountOfERC20), ItemType.ERC20, testTokenSecond.address),
		];
		...
		expect((await testToken.balanceOf(interactor.address)).toString()).
			equal(interactorBalanceExpecrted);
	...
	it("Should make OFFER_TO_CURRENCY with ERC721 token", async function () {
		...
		let tokenId = 0
		buyEvents.forEach(x => {
			expect(x.args[0]).equal(interactor.address)
			expect(x.args[1]).equal(signer.address)
			expect(x.args[2]).equal(testCollectionAsk.address)
			expect(x.args[3]).equal(tokenId.toString())
			expect(x.args[4]).equal("1")
			tokenId++;
		})
	...
	it("Should make BUNDLE_TO_CURRENCY ", async function () {
		...
		let interactorBalanceExpected = "9825";
		let marketplaceBeneficiaryBalanceExpected = "50";
		let creatorExpectedBalance = "25";
		let signerExpectedBalance = "10100";
		...
		const bid: Asset[] = [
			new Asset(0, ethers.BigNumber.from(1), ItemType.ERC721, testCollectionBid.address),
			new Asset(1, ethers.BigNumber.from(1), ItemType.ERC721, testCollectionBid.address),
			new Asset(2, ethers.BigNumber.from(1), ItemType.ERC721, testCollectionBid.address),
		];
