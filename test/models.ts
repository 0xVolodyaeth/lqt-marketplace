...
export class Asset {
	public id: number;
	public amount: ethers.BigNumber;
	public itemType: ItemType;
	public collection: string;

	constructor(id: number, amount: ethers.BigNumber, itemType: number, collection: string) {
		this.id = id;
		this.amount = amount;
		...
		if (this.bidAny && this.bid.length === 1) {
			return ethers.utils.keccak256(ethers.utils.defaultAbiCoder.encode(
				[
					...
					"address",
					"uint256",
					"tuple(uint256 itemType, address collection, int256 id, uint256 amount)[]",
					"uint256",
					"uint256",
					"uint256",
				],
