...
contract MarketplaceV2_5 is Ownable {
    ...
    function changeCollectionRoyalties(
        ...
        require(
            !IERC165(_collection).supportsInterface(_INTERFACE_ID_FEES),
            "Collection haw own royalties"
        );
    ...
    function cancel(uint256 _id) external {
        require(
            msg.sender == positions[_id].owner || msg.sender == owner(),
            "Access denied"
        );
    ...
    function transferFees(uint256 _price, Position memory position)
        ...
        returns (uint256)
    {
        HasSecondarySaleFees collection = HasSecondarySaleFees(
            position.nft.collection
        );
        uint256 result = 0;
        if (
            ...
            address payable[] memory recipients = collection.getFeeRecipients(
                position.nft.id
            );
            ...
            uint256 fee = getFee(
                _price,
                customCollectionRoyalties[position.nft.collection].fee
            );
            transfer(
                customCollectionRoyalties[position.nft.collection].recipient,
                position.currency,
                fee
            );
            result = result + fee;
